    ...
    public function drawCaption($position) {
        ...
        $this->chart->canvas->addCode(
            '<text x="' . $coord[0] . '" y="' . ($coord[1] + $fontSize / 3) . '">'
                ...
                ($this->section->showPointInfo ? ('<tspan dx="' . ($this->isRetro() ? '-6' : '') . '" dy="' . (-$fontSize / 3) . '" class="vul-point-info-caption">'
                    . (floor(radDeg($position) % 30))) : '') . '</tspan></text>');
    }

    public function draw() {
        $this->chart->canvas->beginGroup(
            [self::DEFAULT_CSS_CLASS, $this->class . ($this->selected ? ' vul-point-selected' : '')], 'data-id="' . $this->info->id . '"');
