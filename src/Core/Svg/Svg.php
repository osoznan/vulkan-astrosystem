...
class Svg {
    ...
    public function begin($id, $params = []) {
        $this->code = '
            <!-- Vulkan AstroSystem / http://astrolog-online.net / meraponimaniya@mail.ru -->
            <div id="' . $id . '" class="vul-container"><svg class="vul-container id="' . $id
    ...
    public function beginGroup($classes = null, $params = '') {
        if ($classes) {
            if (is_array($classes)) {
                $classTxt = 'class="' . implode(' ', $classes) . '"';
            } else {
                $classTxt = is_string($classes) ? ('class="' . $classes . '" ') : ' ';
            }
    ...
    public function text($x, $y, $text, $class='', $params = '') {
        $this->code .= "<text x=\"{$x}\" y=\"{$y}\" {$params}" . ($class ? (' class="' . $class . '"') : '') . ">{$text}</text>";
    }
